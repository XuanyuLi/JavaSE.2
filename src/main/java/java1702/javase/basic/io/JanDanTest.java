 ...
 * on 2017/4/21.
 */
public class JanDanTest implements Runnable{
    private static final String PAGE_URL = "http://jandan.net/tag/%E5%A4%A9%E6%96%87/page/";
    private int page;

    public JanDanTest(int page) {
        this.page = page;
    }

    public static void main(String[] args) {
        for (int i = 0; i < 18; i++) {
            Thread thread = new Thread(new JanDanTest(i + 1));
            thread.start();
    ...
    private static void download(String imageUrl, String extension, int page, int counter) {
        try {
            ...
            try (
                    ...
                    BufferedWriter out = new BufferedWriter(new FileWriter("images/" + page + "-" + counter + extension))
            ) {
                int i;
                while ((i = in.read()) != -1) {
                    out.write(i);
